...
export default function InvestmentsClient({ initialInvestments }: { initialInvestments: Investment[] }) {
  ...
  const TYPE_META: Record<InvestmentType, { label: string; icon: string; color: string }> = {
    savings:      { label: prefs.language === 'en' ? 'Savings'      : 'Poupança',         icon: '🏦', color: '#22c55e' },
    high_yield:   { label: prefs.language === 'en' ? 'High Yield'   : 'Conta Rendimento', icon: '💰', color: '#3b82f6' },
    stocks:       { label: prefs.language === 'en' ? 'Stocks'       : 'Ações',            icon: '📈', color: '#8b5cf6' },
    fixed_income: { label: prefs.language === 'en' ? 'Bonds'        : 'Renda Fixa',       icon: '📄', color: '#f59e0b' },
    crypto:       { label: prefs.language === 'en' ? 'Crypto'       : 'Cripto',           icon: '🪙', color: '#f97316' },
  ...
  async function handleDelete(id: string) {
    if (!confirm(prefs.language === 'en' ? 'Delete this investment?' : 'Deseja excluir este investimento?')) return
    setDeletingId(id)
  ...
  const allocation = prefs.language === 'en' ? 'Allocation by type' : 'Alocação por tipo'
  const assets     = prefs.language === 'en' ? 'Your assets' : 'Seus ativos'
  const addNew     = prefs.language === 'en' ? `+ ${tr.addInvestment}` : `+ Novo investimento`
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {investments.length} {prefs.language === 'en' ? `asset${investments.length !== 1 ? 's' : ''}` : `ativo${investments.length !== 1 ? 's' : ''}`}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
        <div className="lg:col-span-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">{allocation}</h2>
          {byType.length === 0 ? (
        ...
        <div className="lg:col-span-3 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">{assets}</h2>
          </div>
